...
# h = 1/(1+exp(-w^T*x))
def sigmoid(x, w):
    return 1/(1+np.exp(-np.dot(vectorX(x),w)))
...
# Optimise w for the loss function
def optimise_w(x, y):
...
def predict(x, w, threshold):
    ...
    discriminant = []
    for i in range(len(x)):
        if sigmoid(x[i], w) > threshold:
            y.append(1)
            discriminant.append(sigmoid(x[i], w))
        else:
            y.append(0)
            discriminant.append(sigmoid(x[i], w))
    return y, discriminant
...
if __name__ == "__main__":
    ...
    plt.savefig('./HW3/LLF.png', dpi=700)
    plt.clf()
    plt.close()
